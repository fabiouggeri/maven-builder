...
public class DependencyResolver {
   ...
   public Artifact resolveDependency(final MavenProject project, final Dependency dependency) throws MojoExecutionException {
      final Artifact artifact = createArtifact(project, dependency);
   ...
   private Artifact createArtifact(final MavenProject project, final Dependency dependency) {
      Map<String, Artifact> map = project.getManagedVersionMap();
      if (dependency.getVersion() == null || dependency.getVersion().isEmpty()) {
         final String depId = dependency.getGroupId() + 
                 ":" + dependency.getArtifactId() + 
                 ":" + dependency.getType() + 
                 (dependency.getClassifier() != null ? ":" + dependency.getClassifier() : "");
         final Artifact artifact = map.get(depId);
         if (artifact != null) {
      ...
      if (pomArtifact.isResolved()) {
         ...
         try { 
            ...
            for (Dependency dep : pomModel.getDependencies()) {
              final Artifact depArtifact = createArtifact(project, dep);
               if (log != null && log.isDebugEnabled()) {
                  log.debug("Depndency found: " + artifact.getId() + ".");
               }
            ...
            throw new MojoExecutionException("Error reading artifact POM " + artifact.getId(), ex);
         }
      }
