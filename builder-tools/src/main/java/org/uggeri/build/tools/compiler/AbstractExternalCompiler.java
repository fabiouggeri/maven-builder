...
public abstract class AbstractExternalCompiler extends AbstractExternalTool<CompilationResult, CompilationRequest> implements Compiler {
      ...
      if (command != null) {
         ...
         try {
            ...
            if (log.isDebugEnabled()) {
               log.debug("PATH env. var : " + path.toString());
            ...
            try {
               ...
               stdoutConsumer.consumeLine("External compiler error: " + ex.getMessage());
               if (ex.getCause() != null) {
         ...
         outFile = null;
         log.error("Command line build error!");
      }
   ...
   private List<File> getOutputFiles(List<File> sourceFiles, AbstractCompilationRequest config) {
      final List<File> outFiles = new ArrayList<>(sourceFiles.size());
